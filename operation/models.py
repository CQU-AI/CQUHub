...
class Topic_Reply(models.Model):
    user = models.ForeignKey(User_Info, on_delete=models.CASCADE, verbose_name='进行回复操作的用户')
    comment = models.ForeignKey(Topic_Comment, on_delete=models.CASCADE, verbose_name='关联评论')
    content = models.TextField(verbose_name='评论内容')
    add_time = models.DateTimeField(auto_now_add=True, verbose_name='评论时间')
